<template>
  <div class="pt-48 bg-gray-100">
    <div class="mx-24">
      <p class="text-2xl uppercase font-semibold">Contact us</p>
      <div class="flex mt-10">
        <div class="w-1/3 px-8">
          <div class="text-sm">
            <p class="text-xl font-semibold">Office Adress</p>
            <p class="text-sm">29 campbell street freetown sierra leone</p>
          </div>
        </div>
        <div class="w-1/3 px-8">
          <div class="text-sm">
            <p class="text-xl font-semibold">Email Adress</p>
            <p>isho@arsabi.com</p>
            <p>turayalhassan11@gmail.com</p>
          </div>
        </div>
        <div class="w-1/3 px-8">
          <div class="text-sm">
            <p class="text-xl font-semibold">Phone Number</p>
            <p>+232 77392736</p>
            <p>+232 78443264</p>
          </div>
        </div>
      </div>
    </div>
    <div class="bg-gray-300 px-24 py-16 text-sm mt-20">
      <div class="w-2/3">
        <p class="text-2xl uppercase font-semibold">contact form</p>
        <div class="flex mt-10 text-sm">
          <p class="w-32 text-right pr-3">Your Name *</p>
          <input
            type="text"
            placeholder="Your Name"
            class="w-full p-2 border-gray-500 ml-5 rounded border text-sm"
          />
        </div>
        <div class="flex mt-5 text-sm">
          <p class="w-32 text-right pr-3">Your Email *</p>
          <input
            type="text"
            placeholder="Your Email"
            class="w-full border-gray-500 p-2 ml-5 rounded border text-sm"
          />
        </div>
        <div class="flex mt-5 text-sm">
          <p class="w-32 text-right pr-3">Your Subject *</p>
          <input
            type="text"
            placeholder="Email Subject"
            class="w-full border-gray-500 p-2 ml-5 rounded border text-sm"
          />
        </div>
        <div class="flex mt-5">
          <p class="w-32 text-right pr-3">Your Message *</p>
          <textarea
            placeholder="Write Your Message"
            class="w-full border-gray-500 p-2 ml-5 rounded border text-sm"
          ></textarea>
        </div>
        <div class="mt-5 flex justify-end">
          <button
            class="w-32 p-2 rounded ml-20 bg-green-500 border border-gray-500 text-white uppercase font-semibold"
          >send</button>
        </div>
      </div>
    </div>
  </div>
</template>

<script>
export default {
  data() {
    return {
      title: "Do D Talk | Contact Us"
    };
  },
  head() {
    return {
      title: this.title,
      meta: [
        {
          hid: "description",
          name: "description",
          content: "My custom description"
        }
      ]
    };
  }
};
</script>
